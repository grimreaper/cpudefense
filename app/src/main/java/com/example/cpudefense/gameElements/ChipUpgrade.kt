    ...
    private var posX: Int, private var posY: Int, val color: Int): Movable
{
    ...
    /** for upgrades of the MAXOUT type: the requested update count */
    private var maxUpdateCount = 0
    ...
    /** returns the maximal number of updates that can be bought with the [availableCash] */
    fun maxPowerUp(availableCash: Int): Int
    {
        var updateCount = 1
    ...
    /** calculates and returns the price that must be paid for this upgrade. */
    {
        ...
        when (type){
            ...
            Chip.ChipUpgrades.MAXOUT -> {
                maxUpdateCount = maxPowerUp(gameMechanics.state.cash).coerceAtLeast(2)
                return powerupPrice(chipToUpgrade.chipData.value, maxUpdateCount)
            }
    ...
    /** adds one or more levels to the chip.
     * @param isAutoGenerated true if the upgrade is done by the game (for chips created at start of stage).
     * In this case, the internal resell value of the chip is limited to a fixed value.
      */
    fun upgradeChip(timesToUpgrade: Int = 1, isAutoGenerated: Boolean)
    {
        chipToUpgrade.addPower(timesToUpgrade)
        chipToUpgrade.chipData.value += price
        // if the chip is created by the game, refund price is less than the value
        if (isAutoGenerated)
            chipToUpgrade.chipData.refund += GameMechanics.valueOfAutogeneratedChip * timesToUpgrade
        else
            chipToUpgrade.chipData.refund += price // resell price is equal to chip value
    }
     ...
     */
    {
        when (type)
        {
            Chip.ChipUpgrades.POWERUP -> upgradeChip(1, doForFree)
            Chip.ChipUpgrades.REDUCE ->
            {
            ...
            Chip.ChipUpgrades.MAXOUT -> upgradeChip(maxUpdateCount, false)
        }
        if (!doForFree) {
    ...
    fun display(canvas: Canvas)
    {
        ...
        val text = when(type)
        {
            ...
            Chip.ChipUpgrades.MAXOUT -> "+$maxUpdateCount"
        }
